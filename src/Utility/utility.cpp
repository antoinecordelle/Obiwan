...
Utility::WindowPtr Utility::initializationBaseWindow(int height, int width, int startY, int startX, const std::string &text, bool center, bool withBox, bool title) {
    // Tool method to initialize windows
    Utility::WindowPtr win(newwin(height, width, startY, startX), delwin);
    if(withBox) {
        box(win.get(), 0 , 0);
        if(title)
            mvwprintw(win.get(), 0, 1, text.c_str());
        else if(!center)
            mvwprintw(win.get(), 1, 1, text.c_str());
        else
            mvwprintw(win.get(), 1, max(COLS/2 - 6, 0), text.c_str());
    }
    else
        mvwprintw(win.get(), 0, 0, text.c_str());
    wrefresh(win.get());
    return win;
}
